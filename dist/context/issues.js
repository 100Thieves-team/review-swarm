 ...
 * says `Fixes MOI-416`.
 */
export function extractIssueKeys(pr, pattern, max) {
    let regex;
    try {
 ...
 */
export async function fetchLinkedIssues(pr, config, logger, env = process.env) {
    if (!config.enabled)
        return [];
    ...
    if (out.length > 0)
        logger.info(`linked issues: ${out.map((issue) => issue.identifier).join(', ')}`);
    return out;
}
