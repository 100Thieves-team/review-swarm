...
export function findingMarker(finding) {
...
export function renderFindingBody(finding, agent, options) {
    ...
    if (finding.suggestion_patch) {
        parts.push(canUseSuggestion(finding)
    ...
    if (finding.verdictReason)
        meta.push(`조정자: ${finding.verdictReason}`);
    ...
    if (meta.length > 0)
        parts.push(`<sub>${meta.join('<br>')}</sub>`);
    parts.push(findingMarker(finding));
    return parts.join('\n\n');
}
/** A GitHub suggestion replaces exactly the commented lines — only offer one when they match. */
export function canUseSuggestion(finding) {
    const anchor = finding.anchor;
    if (!anchor || anchor.side !== 'RIGHT' || anchor.snappedBy !== 0)
        return false;
    if (anchor.line !== finding.end_line)
        return false;
...
export function renderSummary(input) {
    const { context, registry, routing, results, outcome, mediatorSummary, skipped, dismissed, durationMs, degraded } = input;
    ...
    parts.push(`| 판정 | 건수 |
| --- | --- |
${['REQUEST_CHANGE', 'SUGGESTION', 'FOLLOW_UP', 'QUESTION']
...
| 합계 | ${total} |`);
    const agentRows = results
        ...
        const kept = [...outcome.inline, ...outcome.summaryOnly].filter((finding) => finding.agents.includes(result.agentId)).length;
...
${renderFindingBody(finding, agent, { includeAgentHeader: true })}
    ...
    if (skipped.length > 0) {
        parts.push(`<details><summary>이전 리뷰에서 이미 지적한 항목 (${skipped.length})</summary>\n\n${skipped
            .map((finding) => `- \`${finding.file}:${finding.anchor?.line ?? finding.start_line}\` — ${escapeHtml(finding.title)}`)
            .join('\n')}\n\n</details>`);
    }
    if (dismissed.length > 0) {
        parts.push(`<details><summary>작성자가 닫은 항목 — 다시 올리지 않음 (${dismissed.length})</summary>\n\n${dismissed
            .map((finding) => `- \`${finding.file}\` — ${escapeHtml(finding.title)}`)
            .join('\n')}\n\n</details>`);
    }
    parts.push('<sub>같은 지적이 반복되면 해당 코멘트 스레드를 <b>Resolve conversation</b> 하거나 👎를 누르세요. 이 PR에서 다시 올라오지 않습니다.</sub>');
    if (outcome.notes.length > 0) {
        parts.push(`<details><summary>정책 게이트 조정 내역 (${outcome.notes.length})</summary>\n\n${outcome.notes
            .map((note) => `- ${note}`)
            .join('\n')}\n\n</details>`);
    }
    ...
    parts.push(`<sub>run \`${context.runId}\` · head \`${context.pr.headSha.slice(0, 7)}\` · 변경 파일 ${context.changedFiles.length}개 · 로컬 멀티에이전트 리뷰</sub>`);
