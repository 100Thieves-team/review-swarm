 ...
 */
export function route(config, registry, changedFiles, diff) {
    const reasons = new Map();
    const add = (agentId, reason) => {
        if (!registry.has(agentId))
            return;
        if (registry.get(agentId)?.klass === 'mediator')
            return;
    ...
    if (fullSweep) {
        for (const agent of registry.values()) {
            if (agent.klass !== 'mediator')
                add(agent.id, `대형 변경(${changedLines}줄)`);
        }
    }
    ...
    for (const dropped of ranked.slice(config.router.maxAgents))
