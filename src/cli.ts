...
import { loadConfig, type EngineName } from './config.ts';
import { createEngine } from './engine/index.ts';
import { runReview } from './orchestrator.ts';
import { readAppCredentials } from './publish/github.ts';
import { run } from './util/exec.ts';
import { createLogger, setLogLevel, type LogLevel } from './util/logger.ts';
...
async function main(): Promise<number> {
  ...
  if (values['log-level']) setLogLevel(values['log-level'] as LogLevel);
...
async function reviewCommand(args: {
  ...
  const repoSlug = (values.repo as string | undefined) ?? process.env.GITHUB_REPOSITORY;
  ...
  const prNumber = resolvePrNumber(values.pr as string | undefined);
  if (!prNumber) {
  ...
  const result = await runReview({
    ...
    ...(values.out ? { outDir: values.out as string } : {}),
    ...(values.base ? { baseOverride: values.base as string } : {}),
    ...(engineOverride ? { engineOverride } : {}),
    ...(values['api-url'] ? { apiUrl: values['api-url'] as string } : {}),
  });

  const headline =
    `${result.outcome.event} · 인라인 ${result.outcome.inline.length}건 · 요약 ${result.outcome.summaryOnly.length}건 ` +
...
function resolvePrNumber(explicit: string | undefined): number | null {
  ...
  try {
    ...
    const candidate =
      payload.pull_request?.number ?? payload.number ?? (payload.issue?.pull_request ? payload.issue.number : undefined);
...
function writeStepSummary(result: Awaited<ReturnType<typeof runReview>>, headline: string): void {
  ...
  if (config.publish.mode === 'apps') {
    for (const agent of registry.values()) {
      ...
      process.stdout.write(
        `${credentials ? '✅' : '➖'} GitHub App \`${agent.id}\` (${agent.appEnvPrefix}_APP_ID / _PRIVATE_KEY)${credentials ? '' : ' — 미설정, 공용 토큰으로 게시됩니다'}\n`,
      );
    }
  }
...
async function initCommand(args: {
  ...
  const files: { target: string; source: string }[] = [
