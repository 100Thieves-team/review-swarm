...
export const VERDICTS = ['REQUEST_CHANGE', 'SUGGESTION', 'FOLLOW_UP', 'QUESTION', 'DROP'] as const;
export type Verdict = (typeof VERDICTS)[number];

/** How much authority a persona has over the merge decision. */
export const AGENT_CLASSES = ['gate', 'analyst', 'value', 'mediator'] as const;
...
/** A finding exactly as an expert persona emitted it. */
...
/** A finding after the harness has attached provenance, an anchor and a fingerprint. */
export interface Finding extends RawFinding {
  id: string;
  /** Persona ids that produced (or were merged into) this finding. */
  agents: string[];
  /** Primary owner — decides which bot posts it and which policy tier applies. */
  owner: string;
  fingerprint: string;
  anchor: Anchor | null;
  ...
  /** Body the mediator rewrote, when it merged several agents into one comment. */
...
export interface Anchor {
  ...
  /** Distance in lines between the requested line and the anchorable one. */
  snappedBy: number;
}
...
export interface DebateRecord {
  ...
  positions: { agent: string; position: string; concession: string; counterProposal: string }[];
}
      ...
      items: {
        ...
        properties: {
          ...
          suggestion_patch: {
            type: ['string', 'null'],
            description:
              'Optional replacement text for exactly the lines start_line..end_line, rendered as a GitHub suggestion block. Null when a patch would be guesswork.',
          },
        },
      },
    },
