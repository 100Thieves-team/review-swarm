...
import type { PriorFinding } from '../publish/github.ts';
import { MEDIATOR_SCHEMA, VERDICTS, type Finding, type ReviewContext, type Verdict } from '../types.ts';
import { asArray, asEnum, asRecord, asString } from '../util/json.ts';
...
export interface MediateOptions {
  ...
  findings: Finding[];
  /** What this swarm already posted on the PR, and what the author closed. */
  prior: PriorFinding[];
  logger: Logger;
}
 ...
 */
export async function mediate(options: MediateOptions): Promise<MediationResult> {
  ...
  for (const entry of asArray(record['decisions'])) {
    ...
    if (!finding) continue;
    finding.verdict = asEnum(decision['verdict'], VERDICTS, 'SUGGESTION') as Verdict;
    ...
    .map((finding) => {
      ...
      if (finding.debate) {
        ...
        for (const position of finding.debate.positions) {
          lines.push(
            `  - \`${position.agent}\`: ${truncate(position.position, 400)}` +
...
function applyFallbackVerdicts(
  findings: Finding[],
  registry: Map<string, AgentDefinition>,
  config: SwarmConfig,
): void {
