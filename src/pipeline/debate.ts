...
import { DEBATE_SCHEMA, SEVERITY_RANK, type Finding, type ReviewContext } from '../types.ts';
...
interface DebatePair {
  finding: Finding;
  challengerId: string;
  /** The opposing finding, when the conflict is between two real findings. */
  counterFinding: Finding | null;
}
 ...
 */
export async function runDebates(options: DebateOptions): Promise<void> {
  const { config, pool, registry, parsed, context, findings, selected, logger } = options;
  if (!config.debate.enabled || findings.length === 0) return;
  ...
  await mapLimit(pairs, Math.max(1, Math.floor(config.engine.concurrency / 2)), async (pair) => {
    ...
    if (!owner || !challenger) return;

    const focusedDiff = fileDiff(parsed, pair.finding.file);

    const speak = async (speaker: AgentDefinition, opposing: AgentDefinition, stance: string) => {
    ...
    const positions = [ownerTurn, challengerTurn].filter((turn): turn is NonNullable<typeof turn> => turn !== null);
  ...
  persist(
    context.runDir,
    'debate.json',
    findings.filter((finding) => finding.debate).map((finding) => ({ id: finding.id, title: finding.title, debate: finding.debate })),
  );
}
...
function selectPairs(
  ...
  for (const finding of findings) {
    if (pairs.length >= config.debate.maxPairs) break;
