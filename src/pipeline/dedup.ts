...
import { DEFAULT_ANCHOR_OPTIONS, resolveAnchor, type ParsedDiff } from '../context/diff.ts';
import type { SwarmConfig } from '../config.ts';
import { SEVERITY_RANK, type ExpertResult, type Finding, type RawFinding } from '../types.ts';
import { normalizeText, shortId, similarity } from '../util/text.ts';

const CLASS_AUTHORITY: Record<AgentDefinition['klass'], number> = {
  gate: 3,
  analyst: 2,
  value: 1,
  mediator: 0,
 ...
 */
export function dedupeFindings(options: DedupOptions): Finding[] {
  ...
  for (const finding of findings) {
    ...
    if (target) mergeInto(target, finding, registry);
...
function isSameIssue(a: Finding, b: Finding): boolean {
...
function rangesOverlap(a: RawFinding, b: RawFinding): boolean {
  return a.start_line <= b.end_line && b.start_line <= a.end_line;
}

function mergeInto(target: Finding, incoming: Finding, registry: Map<string, AgentDefinition>): void {
  for (const agentId of incoming.agents) {
    if (!target.agents.includes(agentId)) target.agents.push(agentId);
  }
  ...
  if (SEVERITY_RANK[incoming.severity] > SEVERITY_RANK[target.severity]) target.severity = incoming.severity;
  ...
  if (!target.anchor && incoming.anchor) target.anchor = incoming.anchor;
...
function authority(registry: Map<string, AgentDefinition>, agentId: string): number {
