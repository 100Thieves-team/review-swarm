...
import { fileDiff, type ParsedDiff } from '../context/diff.ts';
...
import { asBool, asEnum, asNumber, asRecord, asString, clamp } from '../util/json.ts';
 ...
 * sees it.
 */
export async function verifyFindings(options: VerifyOptions): Promise<Finding[]> {
  const { config, pool, parsed, context, findings, logger } = options;
  if (!config.verify.enabled) return findings;
  ...
  await mapLimit(targets, config.engine.concurrency, async (finding) => {
    ...
    const votes = await mapLimit(
      Array.from({ length: config.verify.voters }, (_, index) => index),
      config.verify.voters,
      async (index) => {
    ...
    const counted = votes.filter((vote): vote is NonNullable<typeof vote> => vote !== null);
    if (counted.length === 0) {
    ...
    const severities = counted.map((vote) => vote.severity).filter((value): value is Severity => value !== null);
    const confidences = counted.map((vote) => vote.confidence).filter((value): value is number => value !== null);
    ...
    if (adjustedSeverity) finding.severity = adjustedSeverity;
    if (adjustedConfidence !== null) finding.confidence = Math.min(finding.confidence, adjustedConfidence);
  });

  const survivors = findings.filter((finding) => !finding.verification?.refuted);
  const dropped = findings.length - survivors.length;
  if (dropped > 0) logger.info(`verify: ${dropped} findings refuted and dropped`);
  persist(context.runDir, 'verify.json', findings.map((f) => ({ id: f.id, title: f.title, verification: f.verification })));

  return survivors;
}

function buildVerifyPrompt(finding: Finding, focusedDiff: string, context: ReviewContext): string {
