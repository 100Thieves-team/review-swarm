 ...
 */
export function renderBlackboard(context: ReviewContext, config: SwarmConfig, prior: PriorFinding[] = []): string {
  const { pr } = context;
...
${prior
  .map(
    (entry) =>
      `- \`${entry.agent}\` ${entry.path ?? '?'}${entry.line ? `:${entry.line}` : ''} — ${entry.title}${entry.dismissed ? ' **(작성자가 닫음)**' : ''}`,
  )
  .join('\n')}
...
export function persist(runDir: string, relativePath: string, data: unknown): string {
