import type { Anchor, ChangedFile, DiffSide } from '../types.ts';
...
export interface Hunk {
  ...
  newLines: number;
  header: string;
  /** Raw hunk body lines including the leading ' ', '+', '-' or '\' marker. */
  lines: string[];
}
...
export interface ParsedFile {
  ...
  additions: number;
  deletions: number;
  /** New-file line numbers that GitHub will accept with side=RIGHT. */
  rightLines: Set<number>;
  /** Old-file line numbers that GitHub will accept with side=LEFT. */
  leftLines: Set<number>;
  /** New-file line numbers actually added by this PR — the preferred anchors. */
  addedLines: Set<number>;
  /** Old-file line numbers removed by this PR. */
...
export function parseUnifiedDiff(diff: string): ParsedDiff {
  ...
  for (const raw of lines) {
    if (raw.startsWith('diff --git ')) {
      if (current) files.set(current.path, current);
    ...
    if (raw.startsWith('deleted file mode')) {
      ...
      if (pendingOldPath) file.path = pendingOldPath;
      continue;
    }
    ...
    if (raw.startsWith('+++ ')) {
      if (raw.slice(4).trim() === '/dev/null') file.status = 'deleted';
      continue;
    }

    const hunkMatch = HUNK_RE.exec(raw);
    if (hunkMatch) {
      const parsedHunk: Hunk = {
...
export function changedFiles(parsed: ParsedDiff): ChangedFile[] {
...
export interface AnchorOptions {
  /** How far a finding's line may be moved to reach a line that exists in the diff. */
  maxSnapDistance: number;
  /** Allow multi-line comments when both ends anchor cleanly. */
...
export function resolveAnchor(
  ...
  if (!target || target.status === 'binary') return null;

  const preferredSide: DiffSide = target.status === 'deleted' ? 'LEFT' : side;
  const order: DiffSide[] = preferredSide === 'LEFT' ? ['LEFT', 'RIGHT'] : ['RIGHT', 'LEFT'];
  ...
  for (const trySide of order) {
    ...
    if (snappedBy > options.maxSnapDistance) continue;

    let start: number | null = null;
    if (options.allowMultiLine && wantedStart < wantedEnd) {
      const candidate = nearest(available, wantedStart);
      if (candidate !== null && candidate < line && line - candidate <= 50) start = candidate;
    }
...
function findBySuffix(parsed: ParsedDiff, path: string): ParsedFile | null {
  const matches = [...parsed.files.values()].filter(
    (file) => file.path.endsWith(`/${path}`) || path.endsWith(`/${file.path}`),
  );
  return matches.length === 1 ? (matches[0] as ParsedFile) : null;
}

/** The diff for one file, reassembled — used to give a verifier a focused context. */
export function fileDiff(parsed: ParsedDiff, path: string): string {
...
function renderFile(file: ParsedFile): string {
  ...
  if (file.status === 'binary') parts.push('Binary files differ');
...
export function renderDiff(parsed: ParsedDiff, paths?: readonly string[]): string {
  const keep = paths ? new Set(paths.map(normalizePath)) : null;
  const parts: string[] = [];
  for (const file of parsed.files.values()) {
    if (keep && !keep.has(file.path)) continue;
