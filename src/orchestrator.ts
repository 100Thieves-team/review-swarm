...
import { buildRegistry } from './agents/registry.ts';
import { loadConfig, type EngineName, type SwarmConfig } from './config.ts';
import { persist, renderBlackboard } from './context/blackboard.ts';
import { collectContext } from './context/collect.ts';
import { EnginePool } from './engine/index.ts';
import { dedupeFindings } from './pipeline/dedup.ts';
import { runDebates } from './pipeline/debate.ts';
import { runExperts } from './pipeline/experts.ts';
import { mediate } from './pipeline/mediator.ts';
...
import { createLogger, group, type Logger } from './util/logger.ts';
...
export interface ReviewOptions {
  ...
  token: string | null;
  apiUrl?: string;
  /** Skip the API call when the workflow already has the payload. */
...
export async function runReview(options: ReviewOptions): Promise<ReviewRunResult> {
  ...
  const pr =
    options.prInfo ??
    (await (async () => {
      if (!token) throw new Error('GitHub 토큰이 필요합니다 (GITHUB_TOKEN) — PR 정보를 가져올 수 없습니다');
  ...
  const { context, parsed, ignoredFiles } = await group('컨텍스트 수집', () =>
    collectContext({ config, workdir, runDir, runId, pr, ...(options.baseOverride ? { baseOverride: options.baseOverride } : {}), logger }),
  );
  ...
  const results = await group('전문가 병렬 실행', () =>
    runExperts({ config, pool, registry, context, blackboard, selected: routing.selected, logger }),
  );
  for (const result of results) {
    if (!result.ok) degraded.push(`\`${result.agentId}\` 실행 실패: ${result.error ?? 'unknown'}`);
  }

  let findings: Finding[] = dedupeFindings({ config, registry, parsed, results });
  ...
  const mediation = await group('조정자 판정', () =>
  ...
  logger.info(
    `policy: ${outcome.inline.length} inline, ${outcome.summaryOnly.length} summary-only, ${outcome.dropped.length} dropped → ${outcome.event}`,
  );

  const publish = await group('GitHub 게시', () =>
    publishReview({
      config,
      context,
      registry,
  ...
  for (const error of publish.errors) logger.error(`publish: ${error}`);
...
function applyOverrides(config: SwarmConfig, options: ReviewOptions): SwarmConfig {
  if (!options.engineOverride) return config;
  return {
    ...config,
    engine: { ...config.engine, default: options.engineOverride },
    agents: Object.fromEntries(
      Object.entries(config.agents).map(([id, agent]) => [id, { ...agent, engine: options.engineOverride }]),
    ),
