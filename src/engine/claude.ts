import type { ClaudeEngineConfig } from '../config.ts';
import { run } from '../util/exec.ts';
import { asRecord, extractJsonObject } from '../util/json.ts';
 ...
 * model prose at all.
 */
export function createClaudeEngine(config: ClaudeEngineConfig): Engine {
  ...
  return {
    ...
    async invoke(request: EngineRequest): Promise<EngineResponse> {
      ...
      if (envelope) {
        ...
        return fail(
          reason ?? `claude returned no structured output${apiError ? ` (api status ${String(apiError)})` : ''}`,
          result.stdout,
          result.durationMs,
        );
      }
      ...
      if (result.code !== 0) {
        return fail(
          `claude exited ${result.code}: ${(result.stderr || result.stdout).trim().slice(0, 600)}`,
          result.stdout,
          result.durationMs,
        );
      }
